...
import com.example.pruebafact.modelos.ErrorJSON;
...
public class ApiController implements Runnable {
    ...
    public ApiController(Context context, String uri, String body, CallbackRsp rspCallback) {
    ...
    private void procesarJuego(JSONObject json) {
        ...
        if (!validacionJuego(juego)) {
            listener.rspListener("{\"response\":\"todo ok\"}", "400");
    ...
    private boolean validacionJuego(Juego juego) {
        if (juego.getTitulo() == null || juego.getNombre() == null) {
            listener.rspListener("{\"response\":\"error en el json\"}", "400");
            return false;
        }
